            ...
            func: async (dataService, context, params) => {
                try {
                    ...
                    if (!["permission", "goal_type", "task"].includes(def_type)) {
                        return ServiceResponseHelper.error("Invalid type. Must be 'permission', 'goal_type', or 'task'.", "INVALID_TYPE");
                    }

                    const res = await dataService.push("business_definitions", {
                        def_type,
                        def_key,
                        def_label,
                    }, context);
                    ...
                    return ServiceResponseHelper.success(`Term ${def_label} (${def_type}) defined successfully.`);
                } catch (e: any) {
            ...
            func: async (dataService, context, params) => {
                try {
                    ...
                    if (!["human", "bot"].includes(type)) {
                        return ServiceResponseHelper.error("Invalid type. Must be 'human' or 'bot'.", "INVALID_TYPE");
                    }

                    const employeeId = crypto.randomUUID();
                    const res = await dataService.push("employees", {
                        id: employeeId,
                        user_id: user_id,
                    ...
                    if (!res.success) return res;
                    return ServiceResponseHelper.success(`Employee ${name} created successfully as ${type}.`, { employee_id: employeeId });
                } catch (e: any) {
                    return ServiceResponseHelper.error(e.message || "Error creating employee", "STAFF_CREATE_ERROR");
                }
            }
        },
        {
            ...
            func: async (dataService, context, params) => {
                try {
                    ...
                    if (!defRes.success || !defRes.data || defRes.data.length === 0) {
                        return ServiceResponseHelper.error(`Permission '${permission_key}' is not defined for this business.`, "UNDEFINED_PERMISSION");
                    }

                    const res = await dataService.write(`employee_permissions[employee_id=${employee_id},permission_key=${permission_key}]`, {
                        granted,
                    }, context);

                    if (!res.success) return res;
                    return ServiceResponseHelper.success(`Permission ${permission_key} updated for employee.`);
                } catch (e: any) {
                    return ServiceResponseHelper.error(e.message || "Error setting permission", "PERM_SET_ERROR");
                }
            }
        },
        {
            ...
            func: async (dataService, context, params) => {
                try {
                    ...
                    const res = await dataService.write(`employee_goals[employee_id=${employee_id},goal_type=${goal_type}]`, {
                        target_value: target,
                        start_date,
                        end_date,
                    }, context);
        ...
        },
        {
            ...
            func: async (dataService, context, params) => {
                try {
                    ...
                    return (res && typeof res === 'object' && 'success' in res) ? res : ServiceResponseHelper.success("Report retrieved", res);
                } catch (e: any) {
