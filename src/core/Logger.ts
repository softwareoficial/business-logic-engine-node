...
class Logger extends EventEmitter {
    constructor() {
        ...
        this.on('log', (event: LogEvent) => {
            ...
            console.log(`${color}[${event.timestamp}] [${event.level}] [${event.source}] ${event.message}`, '\x1b[0m');
            if (event.context) {
    ...
    private createEvent(level: LogLevel, source: ErrorSource, message: string, context?: any): LogEvent {
        return {
